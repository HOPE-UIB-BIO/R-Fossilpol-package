...
util_save_if_latests <- function(
  ...
  if(!exists(system_setting_file, envir = .GlobalEnv)){
    usethis::ui_info(
      paste("File", system_setting_file, "not found.", 
  ...
  if(is.na(latest_file_name)){
    ...
    if(prefered_format == "rds" & has_setting == TRUE){
      file_to_save <- 
        list(
    ...
    if(compress == TRUE){
      save_command <- 
        paste0("write_", prefered_format,"(file_to_save, '", dir, "/",
  ...
  load_command <- 
    paste0("lastest_file <- read_",lastest_file_format,
  ...
  if(any(names(lastest_file) == "data")){
  ...
  if(is_the_lastest_same == FALSE){
    
    usethis::ui_done("Found older file, overwriting it with new changes.")
    
    if(lastest_file_format == "rds" & has_setting == TRUE){
    ...
    if(compress == TRUE){
      save_command <- 
        paste0("write_", lastest_file_format,"(file_to_save, '",
