...
util_compare_files <- 
  function(file_name_a,
    ...
    util_check_class("file_a_envir",
                     c("NULL",
                       "environment"))
    
    util_check_class("file_b_envir",
                     c("NULL",
                       "environment"))
    ...
    if(is.null(file_a_envir)) {
      file_a_envir <- parent_env
    }
    
    if(is.null(file_b_envir)) {
    ...
    # lists
    if(any(file_class_a == "list") | any(file_class_b == "list")){
      # size
      file_length_a <- 
        file_name_a %>% 
        get(., envir = file_a_envir) %>% 
      ...
      if(file_length_a == file_length_b){
        full_list_a <-
          file_name_a %>% 
          get(., envir = file_a_envir) %>%  
          unlist()
        ...
        if(all(full_list_a == full_list_b)){
    ...
    # data frames
    if(any(file_class_a == "data.frame" )| any(file_class_b == "data.frame")){
