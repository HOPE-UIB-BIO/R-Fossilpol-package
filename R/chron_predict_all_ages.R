...
chron_predict_all_ages <- 
  function(data_source,
    ...
    util_output_comment(
      paste("Ages were sucessfully predicted for",
            nrow(data_age_predicted_summary), "out of",
            nrow(data_age_predicted), "sequences"))
    
    # save sites that fail to run
    if(nrow(data_age_predicted_summary) != nrow(data_age_predicted)){
      ...
      if(nrow(sites_fail_to_predict) > 0) {
